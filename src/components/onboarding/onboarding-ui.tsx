...
export function BlueprintBackdrop() {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      <InterviewSignal className="absolute left-1/2 top-8 h-[26rem] w-[34rem] -translate-x-1/2 opacity-10 sm:h-[32rem] sm:w-[42rem]" />
      <div className="absolute inset-0 opacity-30" style={BLUEPRINT_GRID} />
      <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-[#3657b4] to-transparent" />
  ...
  return (
    <div className={["flex min-w-0 items-center gap-3.5 p-4", CARD].join(" ")}>
      ...
      <div className="min-w-0">
        <p className="truncate text-xs font-semibold uppercase text-[#111827]/80">
          {label}
        </p>
...
export function EvidencePanel({
  ...
  return (
    <section className={["overflow-hidden", CARD].join(" ")}>
      <div className="flex items-center gap-3 border-b border-[#13234f]/10 px-5 py-4">
        <span
          className="grid h-11 w-11 shrink-0 place-items-center rounded-lg"
          style={{ backgroundColor: "rgba(17,24,39,0.08)", color: "#111827" }}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          {caption ? <p className="mt-0.5 text-[12.5px] text-[#13234f]/55">{caption}</p> : null}
        </div>
        <span
  ...
  return (
    <div className={["grid place-items-center px-6 py-10 text-center", CARD_INNER].join(" ")}>
      <div className="max-w-sm">
        ...
        <p className="mt-2 text-[13px] leading-6 text-[#13234f]/65">{body}</p>
      </div>
    </div>
  );
}
...
function Eyebrow({ icon: Icon, children }: { icon: LucideIcon; children: ReactNode }) {
  return (
    <span className="inline-flex items-center gap-2.5 rounded-full border border-cream/20 bg-cream/5 px-3.5 py-1.5 backdrop-blur-sm">
      <Icon size={14} className="text-cream" aria-hidden="true" />
