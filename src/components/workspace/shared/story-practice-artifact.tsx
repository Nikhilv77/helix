...
import type { AppliedEngineeringArtifactKind } from "@/lib/practice/applied-engineering/contracts";

export type StoryPracticeArtifactKind =
  | AppliedEngineeringArtifactKind
  | "scenario";
...
/** Typed renderer shared by Core Technical and Applied Engineering evidence artifacts. */
    ...
    <section aria-labelledby="question-artifact-heading">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3 px-0.5">
        <div>
          ...
          <h2 id="question-artifact-heading" className="mt-1.5 text-[16px] font-semibold tracking-[-0.015em] text-cream/88">
            {artifact.title}
          </h2>
          {artifact.caption ? <p className="mt-1 text-[11px] leading-5 text-cream/38">{artifact.caption}</p> : null}
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-white/[0.085] bg-[#0b0d10] shadow-[0_16px_45px_rgba(0,0,0,0.2)]">
        <div className="flex h-10 items-center gap-2 border-b border-white/[0.065] bg-[#15181d] px-3.5">
          {presentation.editor ? <Code2 size={12} aria-hidden="true" className="text-[var(--workspace-accent)]" /> : <FileText size={12} aria-hidden="true" className="text-[var(--workspace-accent)]" />}
          ...
          <span className="ml-auto text-[9.5px] font-semibold uppercase tracking-[0.11em] text-cream/28">Read only</span>
        </div>
        {presentation.editor ? (
          <div style={{ height: codeViewerHeight(artifact.content, 440) }}>
            <DsaCodeEditor language="javascript" value={artifact.content} readOnly autoFocus={false} ariaLabel={`${artifact.title} ${artifact.kind} artifact, read only`} />
          </div>
        ) : (
            ...
            <ol className="min-w-max font-mono text-[12px] leading-[1.85] text-cream/64">
              {lines.map((line, index) => (
                <li key={`${index}:${line}`} className="flex min-h-6 hover:bg-white/[0.025]">
                  <span aria-hidden="true" className="w-12 shrink-0 select-none border-r border-white/[0.045] pr-3 text-right tabular-nums text-cream/20">{index + 1}</span>
...
function artifactPresentation(artifact: StoryPracticeArtifactData): { editor: boolean; extension: string } {
  switch (artifact.kind) {
    ...
    case "logs":
      return { editor: /(^|\n)\s*(?:import|export|const|let|var|function|class|setImmediate\(|setTimeout\()/m.test(artifact.content), extension: "log" };
    case "trace":
