    ...
    <section className="mx-auto w-full max-w-6xl text-cream">
      ...
      <header className="relative mx-auto flex max-w-2xl flex-col items-center pb-12 pt-2 text-center sm:pb-14">
        ...
        <div className="identity-stage-in relative z-10 -mt-7 flex max-w-xl flex-col items-center sm:-mt-9">
          <div className="relative rounded-2xl bg-[#17181b] px-5 py-4 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] sm:px-6">
            <span aria-hidden className="absolute -top-2 left-1/2 h-4 w-4 -translate-x-1/2 rotate-45 bg-[#17181b]" />
            ...
            >
              {speaking || voiceState === "loading" ? <VolumeX size={15} aria-hidden="true" /> : <Volume2 size={15} aria-hidden="true" />}
              {voiceState === "loading" ? "Starting Maya" : speaking ? "Stop Maya" : "Hear Maya"}
            </button>
          </div>
          {awaitingGesture ? <p className="mt-3 text-xs font-medium text-[var(--workspace-accent)]">Tap to hear Maya</p> : null}
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-2 text-sm text-cream/58">
            <Meta icon={MessageSquareText} label="Round" value={roundShortLabel(report.setup.roundType)} />
            ...
            <Meta icon={ArrowUpRight} label="Difficulty" value={capitalize(report.setup.intensity)} />
          </div>
        </div>
      </header>

      <article
        className="report-latest-strip rounded-[1.5rem] bg-[#17181b] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] sm:p-7"
    ...
    <section className="border-t border-white/[0.07] py-7 first:border-t-0 first:pt-0">
      ...
      <div className="mt-4 divide-y divide-white/[0.07]">
        {signals.map((signal, index) => (
          <SignalNote key={`${mode}-${signal.id}`} signal={signal} mode={mode} delay={delay + index * 90} />
        ))}
      </div>
    </section>
  );
...
function SignalNote({ signal, mode, delay }: { signal: Signal; mode: FeedbackMode; delay: number }) {
  const Icon = signal.icon;
  const status = signalStatus(signal.score);
  const title = mode === "next-step" ? `Start with ${signal.label.toLowerCase()}.` : signal.observation;
  const copy = mode === "working" ? signal.evidence : signal.nextMove;
  return (
    <article
      className="report-latest-strip grid gap-4 py-5 sm:grid-cols-[2.75rem_minmax(0,1fr)_6.5rem] sm:items-center"
      style={{ "--report-delay": `${delay}ms` } as CSSProperties}
    >
      <span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--workspace-accent-soft)] text-[var(--workspace-accent)]">
        <Icon size={19} strokeWidth={1.8} aria-hidden="true" />
      </span>
      <div className="min-w-0">
        <p className="text-xs font-medium uppercase tracking-[0.13em] text-[var(--workspace-accent)]">{signal.label}</p>
        <h3 className="mt-1.5 text-lg font-semibold tracking-tight text-cream">{title}</h3>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-cream/58">{copy}</p>
      </div>
      <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:gap-1">
        <span className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-[var(--workspace-accent-soft)] text-center shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--workspace-accent)_28%,transparent)]">
          <span className="text-xl font-semibold leading-none tabular-nums text-cream">{signal.score}</span>
        </span>
        <p className="text-xs font-medium text-[var(--workspace-accent)]">{status}</p>
      </div>
    </article>
  );
}
...
function Meta({ icon: Icon, label, value }: { icon: typeof Timer; label: string; value: string }) {
  return (
    <span className="inline-flex items-center gap-2">
      ...
      <span><span className="text-cream/42">{label}: </span>{value}</span>
    </span>
  );
}
...
function buildSignals(report: InterviewReport): Signal[] {
  ...
  const average = (pick: (item: (typeof answered)[number]) => number) =>
    answered.length ? Math.round(answered.reduce((total, item) => total + pick(item), 0) / answered.length) : 0;
  ...
  return [
    ...
    signal("structure", "Structure", average((item) => item.evidenceBreakdown?.decision ?? item.evidenceScore), ListChecks, report),
    signal("ownership", "Ownership", average((item) => item.evidenceBreakdown?.ownership ?? item.evidenceScore), UserRoundCheck, report),
    signal("impact", "Impact", average((item) => item.evidenceBreakdown?.outcome ?? item.evidenceScore), CheckCircle2, report)
  ];
}
...
function signalNextMove(signalId: Signal["id"]) {
  const nextMoves: Record<Signal["id"], string> = {
    clarity: "Lead with the point, then support it with one concrete detail so Maya never has to ask what you mean.",
    structure: "Use a simple context → decision → result flow to make the path through your answer easy to follow.",
    ownership: "Name the part you personally owned, the choice you made, and why that choice mattered.",
    impact: "Close each answer with the outcome, a metric, or the learning that changed what happened next."
  };
...
function mayaSummary(report: InterviewReport, candidateName: string, strongest: string, gap: string) {
...
function pressureMessage(report: InterviewReport) {
  ...
  if (clarifications > 0) return `A few answers needed a little more context. Leading with your main point will make them easier to follow.`;
  return "Maya could follow your answers without needing extra clarification.";
}

function buildPdfBriefing(report: InterviewReport, overview: ReportsOverview, candidate: Candidate): ReportPdfBriefing {
  const gap = report.summary.recommendedFocus ?? "Answer endings";
  const strongest = report.summary.strongest ?? "Your interview signal";
  return {
    verdict: report.summary.evidenceScore >= 75 ? "Your interview signal is getting strong." : "Your interview signal is developing.",
    trend: overview.scoreDelta && overview.scoreDelta > 0 ? `Your score is up ${overview.scoreDelta} points from your first round.` : "Keep building on the evidence from this round.",
    ...
    history: overview.rounds.slice(0, 4).map((round) => `${roundShortLabel(round.roundType)} · ${round.evidenceScore ?? 0}/100 · ${formatShortDate(round.startedAt)}`),
    readinessScore: overview.readinessScore,
    competencyBars: overview.competencies.slice(0, 4).map((item) => ({ label: item.label, score: item.averageScore, level: item.level })),
