...
type QuestionRead = Prisma.AppliedEngineeringBlockQuestionGetPayload<{
...
export class AppliedEngineeringPracticeService {
  ...
  async historyBlock(ownerId: string, blockId: string) {
    ...
    if (!block)
      throw new NotFoundErrorException(
        ...
        "Applied Engineering incident block not found."
      );
      ...
      assertStoryPracticeHintOrder(
        ...
        () =>
          new ConflictErrorException(
            "APPLIED_ENGINEERING_HINT_OUT_OF_ORDER",
  ...
  async runCode(ownerId: string, rawInput: unknown) {
    ...
    const replay = await this.prisma.appliedEngineeringCodeRun.findUnique({
    ...
    try {
      ...
      throw new ServiceUnavailableErrorException(
        "APPLIED_ENGINEERING_RUNNER_UNAVAILABLE",
        "The isolated Node.js runner is temporarily unavailable. Your draft is safe; try again.",
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const existing = await tx.appliedEngineeringCodeRun.findUnique({
        where: { ownerId_requestId: { ownerId, requestId: input.requestId } },
        select: {
          id: true,
          blockQuestionId: true,
      ...
      const created = await tx.appliedEngineeringCodeRun.create({
        data: {
          ownerId,
          blockQuestionId: input.questionId,
  ...
  async submitAttempt(ownerId: string, rawInput: unknown) {
    ...
    const replay = await this.prisma.appliedEngineeringQuestionAttempt.findUnique({
      where: { ownerId_requestId: { ownerId, requestId: input.requestId } },
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      ...
      const attempt = await tx.appliedEngineeringQuestionAttempt.create({
        data: {
          ownerId,
          blockQuestionId: input.questionId,
          requestId: input.requestId,
  ...
  private async findQuestion(ownerId: string, questionId: string): Promise<QuestionRead> {
    const question = await this.prisma.appliedEngineeringBlockQuestion.findFirst({
  ...
  private async ownedRun(
    ...
    work: Extract<AppliedEngineeringAttemptWork, { kind: "code" }>
  ) {
    const run = await this.prisma.appliedEngineeringCodeRun.findFirst({
...
function publicQuestion(question: QuestionRead) {
  ...
  const answerAuthorized =
    attempted || question.status === AppliedEngineeringQuestionStatus.LEARNED;
  ...
  return {
    ...
    revealedHints: Array.from({ length: hintCount }, (_, index) =>
      revealAppliedEngineeringHint(frozen, (index + 1) as 1 | 2 | 3)
    ),
    authorizedAnswer: answerAuthorized
      ? appliedEngineeringAuthorizedAnswerSchema.parse({
...
const attemptReplaySelect = {
  ...
  score: true,
  createdAt: true
} satisfies Prisma.AppliedEngineeringQuestionAttemptSelect;

function parseDraft(
  value: Prisma.JsonValue | null | undefined
): AppliedEngineeringDraftWork | null {
  return value === null || value === undefined
    ? null
    : appliedEngineeringDraftWorkSchema.parse(value);
}
function publicAttempt(attempt: {
  ...
  return {
    ...
    work: appliedEngineeringAttemptWorkSchema.parse(attempt.answerSnapshot),
    feedback: appliedEngineeringAttemptFeedbackSchema.parse(attempt.evaluationSnapshot),
    verificationStatus: attempt.verificationStatus,
    score: attempt.score,
    createdAt: attempt.createdAt.toISOString()
  };
}
function publicRunResult(value: Prisma.JsonValue): AppliedEngineeringRunResult {
  return appliedEngineeringPublicRunResultSchema.parse(value) as AppliedEngineeringRunResult;
}
function replayRun(
  run: {
    id: string;
  ...
  assertStoryPracticeRunReplay(
    ...
    () =>
      new ConflictErrorException(
        "APPLIED_ENGINEERING_RUN_REQUEST_CONFLICT",
...
function assertWorkMatchesQuestion(
  rawQuestion: unknown,
  work: AppliedEngineeringDraftWork | AppliedEngineeringAttemptWork | null
) {
  if (work === null) return;
  const question = appliedEngineeringQuestionSchema.parse(rawQuestion);
  assertStoryPracticeWorkMatchesQuestion(question, work, {
    ...
    choice: () =>
      new ConflictErrorException(
        "APPLIED_ENGINEERING_CHOICE_INVALID",
...
async function assertOwnedRun(
  ...
  const run = await tx.appliedEngineeringCodeRun.findFirst({
    where: { id: work.runId, ownerId, blockQuestionId: questionId },
    select: { contentFingerprint: true, codeFingerprint: true, resultSnapshot: true }
  });
  validateRunBinding(run, contentFingerprint, work.code);
...
function validateRunBinding(
  ...
  assertStoryPracticeRunBinding(
    ...
    () =>
      new ConflictErrorException(
        "APPLIED_ENGINEERING_RUN_MISMATCH",
...
async function lockBlock(tx: Prisma.TransactionClient, blockId: string): Promise<void> {
...
function questionNotFound() {
  return new NotFoundErrorException(
    "APPLIED_ENGINEERING_QUESTION_NOT_FOUND",
    "Applied Engineering question not found."
  );
}
function questionNotMutable() {
  return new ConflictErrorException(
    "APPLIED_ENGINEERING_QUESTION_READ_ONLY",
    "This Applied Engineering question is no longer active."
  );
}
