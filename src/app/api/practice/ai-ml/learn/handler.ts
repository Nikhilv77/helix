...
export async function POST(request: NextRequest) {
  try {
    ...
    const input = await parseAiMlStoryJson(request, coreTechnicalLearnInputSchema);
    return apiSuccess({ question: await app.aiMlStoryPracticeService.learn(ownerId, input) });
  } catch (error) {
