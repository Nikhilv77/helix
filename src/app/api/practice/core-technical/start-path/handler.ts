...
import { coreTechnicalStartPathInputSchema } from "@/features/practice/core-technical/domain/practice-contracts";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const input = await parseCoreTechnicalJson(request, coreTechnicalStartPathInputSchema);
    lease = await getSharedGuard(app.config).acquire(
      {
        namespace: "core-technical-start-path",
        ttlMs: 300_000,
