...
import { architectureDesignStartPathInputSchema } from "@/features/practice/architecture-design/domain/practice-contracts";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    lease = await getSharedGuard(app.config).acquire(
      {
        namespace: "architecture-design-start-path",
        ttlMs: 300_000,
    ...
    return apiSuccess(await app.architectureDesign.preparation.startPath(ownerId, input));
  } catch (error) {
