-- A successful practice run can be persisted directly as COMPLETED. The
-- previous policy only trusted a SUBMITTED row accompanied by a COMPLETED row,
-- which incorrectly excluded helpers whose full score lived on COMPLETED.
-- Normalize both storage shapes into one solution per question before scoring.
...
COMMENT ON FUNCTION "helpHelperEligibilityScore"(TEXT, TEXT, TEXT) IS
  'Returns 0 for an ineligible helper or a normalized evidence score from exact, pattern, demonstrated-performance, or verified-profile evidence; scored COMPLETED attempts are valid solution evidence.';
