-- Exact Trailgrad completion is excellent evidence, but it is not the only
-- way somebody can know a problem. This function centralizes the evidence
-- lanes used by matching, inbox visibility, and the authoritative claim write.
-- Returning a score rather than a boolean lets exact proof remain a ranking
-- advantage without making it a gate.
...
AS $$
  ...
  profile_evidence AS (
    SELECT CASE
      WHEN profile."resumeVerifiedAt" IS NULL
        OR COALESCE(profile."resumeConfidence", 0) < 0.65
        OR profile."onboardingCompletedAt" IS NULL
      THEN 0
      -- Explicit external DSA/competitive-programming evidence is the clearest
      -- profile path for somebody who solved outside Trailgrad.
      ...
      THEN 0.62 + LEAST(profile."resumeConfidence", 1) * 0.15
      -- An experienced engineer with verified evidence in the requested
      -- language is also allowed, but ranks below DSA-specific evidence.
      WHEN profile."level" IN ('3-5', '5-plus')
        AND CASE p_language
          ...
          ELSE FALSE
        END
      THEN 0.55 + LEAST(profile."resumeConfidence", 1) * 0.12
      ELSE 0
  ...
  SELECT CASE
    ...
    ELSE GREATEST(
      COALESCE(
        (SELECT 0.85 + LEAST(exact_evidence.score, 1) * 0.15 FROM exact_evidence),
        0
      ),
      COALESCE(
        (
...
COMMENT ON FUNCTION "helpHelperEligibilityScore"(TEXT, TEXT, TEXT) IS
  'Returns 0 for an ineligible helper or a 0..1 evidence score from exact, pattern, demonstrated-performance, or verified-profile evidence.';
